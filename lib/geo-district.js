...
// 读 ip_lookups：中文 region/city（cron 用 ip-api lang=zh-CN 查得，永久保存）+ 区县及其检查时间。
// 没有该 IP → null。**不联网**，只 1 次 SELECT；50 条内的 select 结果还会进实例内存缓存。
async function getStoredGeo(rawIp) {
  ...
  try {
    ...
    var r = await pg.query(
      "select region, city, district, district_checked_at from ip_lookups where ip = $1",
      [ip]
    );
    var out = null;
...
try {
  ...
  console.warn("[geo-district] @vercel/functions 不可用，后台任务退化为 fire-and-forget");
}

function runInBackground(promise) {
  var p = Promise.resolve(promise).catch(function (e) {
    console.warn("[geo-district] background task error:", e && e.message);
  });
  if (waitUntilFn) {
    try {
      waitUntilFn(p);
  ...
  // 兜底：不 await。极端情况下这次写入可能被冻结丢掉，下次访问会重试，不影响正确性。
}
