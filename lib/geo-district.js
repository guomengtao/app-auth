...
var API_TIMEOUT_MS = 800;
var CACHE_FRESH_MS = 30 * 24 * 60 * 60 * 1000; // 同一 IP 30 天内不重复调用
...
async function readCache(ip) {
  ...
  if (!ts || Date.now() - ts > CACHE_FRESH_MS) return null; // 过期 → 重查
...
async function getDistrict(rawIp) {
  ...
  var task = (async function () {
    try {
      ...
      if (district === null) return ""; // 失败：不写缓存，下次重试
