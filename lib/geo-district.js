...
async function getStoredGeo(rawIp) {
  ...
  try {
    ...
    var r = await pg.query(
      "select region, city, region_zh, city_zh, district, district_checked_at, " +
        "isp, org, asn, lat, lon, timezone, country, updated_at " +
        "from ip_lookups where ip = $1",
      [ip]
    );
    var out = null;
    if (r && r.rows && r.rows.length) {
      var row = r.rows[0];
      // 按「来源成组」取中文省市：腾讯有数据就整组用腾讯，否则整组用 ip-api，避免混搭矛盾
      var pair = geoZh.pickCnPair(row);
      out = {
        country: String(row.country || "").trim(),
        region: pair.region,
        city: pair.city,
        geoSource: pair.source,
        district: String(row.district || "").trim(),
        checkedAt: row.district_checked_at ? new Date(row.district_checked_at).getTime() : 0,
        fromTencent: pair.source === "tencent",
        // 网络信息（腾讯不提供，只有 ip-api 那条链路有）
...
// ⭐ 同步版：**用户显式查询时**才用（如/我的 IP 页面）——允许阻塞等腾讯（≤2.5s）并落库。
//    与 getDistrict() 的区别：getDistrict 只读缓存 + 后台预热（通知链路不能阻塞）；
//    这里会真的等结果，因为用户就站在页面前等答案。同一 IP 成功后 30 天内不再调用。
async function resolveNow(rawIp, force) {
  var ip = normalizeIp(rawIp);
  if (isPrivateOrInvalid(ip)) return null;

  var stored = await getStoredGeo(ip);
  if (!force) {
    var fresh = districtFromStored(stored);
    if (fresh !== null) {
      return {
        region: (stored && stored.region) || "",
  ...
  var ad = await fetchAdInfo(ip);
  if (ad === null) {
    // 腾讯失败（121 配额未分配 / 110 未授权 / 超时）：返回已有缓存 + 标记失败原因，交给调用方展示
    return stored
      ? {
          region: stored.region, city: stored.city, district: stored.district,
          checkedAt: stored.checkedAt || 0, cached: true, tencentFailed: true,
        }
      : { region: "", city: "", district: "", checkedAt: 0, cached: false, tencentFailed: true };
  }
...
module.exports = {
  ...
  getStoredGeo: getStoredGeo, // 读 ip_lookups 的中文省市/区县/网络信息（1 次 SELECT，不联网）
  resolveNow: resolveNow, // 用户显式查询用（同步等腾讯，仅 /api/visitor/ip 调用）
