...
// 「我的 IP」页面强制刷新限流：1 次/分钟/IP（独立 key，避免和访客埋点/激活互相影响）
var MYIP_FORCE_MAX_HITS = 1;

async function checkMyIpForceLimit(req) {
  var ip = getClientIp(req);
  var key = "ratelimit:myip:" + getMinuteKey() + ":" + ip;
  var count = await redis.incr(key);
  if (count === 1) {
    await redis.pexpire(key, secondsToNextMinute() * 1000).catch(function () {});
  }
  if (count > MYIP_FORCE_MAX_HITS) {
    return { blocked: true, retryAfterMs: secondsToNextMinute() * 1000, reason: "刷新过于频繁，请稍后再试" };
  }
  return { blocked: false };
}
...
module.exports = {
  ...
  checkMyIpForceLimit: checkMyIpForceLimit,
