...
async function processOrder(order) {
  ...
  try {
    ...
    var dmResult = { sent: false, skipped: false, error: null };

    if (afdianApi.isConfigured() && order.user_id) {
      var dmKey = "afdian:dm_sent:" + outTradeNo;
      var alreadySent = await redis.get(dmKey);
      if (alreadySent) {
        console.log("[afdian:processor]   DM already sent for this order (dedup), skip");
        dmResult = { sent: true, skipped: true, reason: "already_sent", sent_at: alreadySent };
      } else {
        try {
          ...
          var msg = "Thank you for your support! Your activation code: " + activationCode + "\n" +
            ...
            "Device ID: " + deviceId;
          var apiResp = await afdianApi.sendMessage(order.user_id, msg);

          if (apiResp && apiResp.ec === 200) {
            var dmSentAt = new Date().toISOString();
            await redis.set(dmKey, dmSentAt);
            orderRecord.dm_sent = true;
            orderRecord.dm_sent_at = dmSentAt;
            orderRecord.dm_response = apiResp.em || "ok";
            await redis.set("afdian:order:" + outTradeNo, JSON.stringify(orderRecord));
            dmResult = { sent: true, sent_at: dmSentAt, response: apiResp.em || "ok" };
            console.log("[afdian:processor]   DM sent successfully, ec=200");
          } else {
            var apiError = (apiResp && apiResp.em) || "Unknown API error, ec=" + (apiResp && apiResp.ec);
            orderRecord.dm_sent = false;
            orderRecord.dm_error = apiError;
            await redis.set("afdian:order:" + outTradeNo, JSON.stringify(orderRecord));
            dmResult = { sent: false, error: apiError };
            console.log("[afdian:processor]   DM API returned error: ec=" + (apiResp && apiResp.ec) + " em=" + (apiResp && apiResp.em));
          }
        } catch (e) {
          orderRecord.dm_sent = false;
          orderRecord.dm_error = e.message;
          await redis.set("afdian:order:" + outTradeNo, JSON.stringify(orderRecord));
          dmResult = { sent: false, error: e.message };
    ...
    console.log("[afdian:processor] processOrder SUCCESS: out_trade_no=" + outTradeNo + " activation_code=" + activationCode + " dm_sent=" + dmResult.sent);
    return {
      success: true,
      out_trade_no: outTradeNo,
      activation_code: activationCode,
