...
// hmacSha256, getProductSalt, computeChecksum are defined above but NOT used here.
// This means anyone with source code can forge a valid-looking 18-digit code
// by calling generateActivationCode() directly (pure math, no server check).
//
// Why we don't fix it now:
//   The activation code is decoded OFFLINE by the smartwatch app. The watch
//   has no internet, so server-side verification won't help. Changing encode
//   to include a signature would break ALL existing activation codes and
//   require a simultaneous watch app update to add verify-sig logic.
//
// When to fix:
//   1. When you can push a watch app update that calls computeChecksum()
//      during decryptActivationCode() to validate the last 2 digits.
//   2. At that point, encode should append checksum to last 2 digits, and
//      decode should verify it before returning valid:true.
//
// Guardrail already added: api/verify-activation.js lets admins verify if
// a code actually exists in Redis (real activation) vs purely mathematical
// (forged). Useful for internal auditing, not for the watch offline flow.
