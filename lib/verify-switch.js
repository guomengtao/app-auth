...
module.exports = async function verifySwitch(req, res) {
  ...
  try {
    ...
    var primaryProvider = primaryFromRedis || String(process.env.DB_PROVIDER || "auto").trim();

    var primaryDb = dbRegistry.getDatabase(primaryProvider);
    if (!primaryDb) {
      var pgDbs = dbRegistry.getPostgresDatabases();
      primaryProvider = pgDbs.length > 0 ? pgDbs[0].id : "upstash";
    }
    ...
    async function getUpstashStats() {
      ...
      try {
        ...
        do {
          ...
          scanRounds++;
        } while (String(cursor) !== "0" && scanRounds < 100);
