...
// ⚠️ 用 mget 而不是 pipeline：lib/redis.js 的 pipeline.exec() 是逐条 await，
//    扫 auth:activation_codes 这种集合会退化成上千次串行查询（接口直接超时）。
async function batchGetByMembers(setKey, prefix, cap) {
  ...
  for (var i = 0; i < members.length; i += BATCH) {
    ...
    var res = await redis.mget(batch.map(function (m) { return prefix ? prefix + m : m; })).catch(function () { return []; });
...
async function resolve(q, opts) {
  ...
  var explicitType = String(opts.type || "").trim() || "auto";
  var anchor;
  if (explicitType !== "auto") {
    var s = String(q || "").trim();
    anchor = { type: explicitType, value: explicitType === "redeem" ? s.toUpperCase() : s };
  } else {
    anchor = await detectAnchor(q);
  }
  ...
  // ⭐ 人工合并规则：优先级高于所有自动证据 —— 把配对的那个身份也当成种子一起捞
  var manualRules = await identity.rulesFor(String(q || "").trim());
  var manualMergeSeeds = [];
  var splitOthers = {};
  manualRules.forEach(function (rule) {
    if (rule.op === "split") splitOthers[rule.other] = rule.kind;
  });
  manualRules.forEach(function (rule) {
    if (rule.op !== "merge") return;
    if (splitOthers[rule.other]) return;   // 拆分优先级最高：同一对值同时有两条规则时不合并
    manualMergeSeeds.push(rule.kind + ":" + rule.other);
    // ⚠️ A 与 B 常常是**不同类型**（最常见：设备 ID ↔ IP，即手表激活 + 电脑浏览），
    //    所以这里按「值的形态」自动归类，而不是照抄规则里记的 kind。
    var v = rule.other;
    if (/^\d{1,3}(\.\d{1,3}){3}$/.test(v) || v.indexOf(":") >= 0) {
      ips[v] = true;
    } else if (/^[A-Z0-9]{4,8}$/.test(v)) {
      redemCodes[v] = true;
      if (!deviceFull) deviceFull = v;
    } else if (v.length >= 8) {
      orderNos[v] = true;
      if (!deviceFull) deviceFull = v;
    } else if (!deviceFull) {
  ...
  // ⭐ 人工拆分规则：禁止把 A 与 B 合并 —— 剔除自动关联出来的属于「另一个人」的事件
  var manualSplitValues = Object.keys(splitOthers).map(function (v) {
    return splitOthers[v] + ":" + v;
  });
  if (manualSplitValues.length) {
    events = events.filter(function (e) {
      var d = e.detail || {};
      var hit = manualSplitValues.some(function (v) {
        var val = v.slice(v.indexOf(":") + 1);
        if (!val) return false;
        // 同样不看 kind：只要事件的任一身份字段等于这个值就剔除
        return String(e.deviceId || "") === val
          || String(e.ip || "") === val
          || String(d.redeem_code || "") === val
          || String(d.out_trade_no || "") === val
          || String(d.activation_code || "") === val;
      });
      return !hit;
    });
  }
  ...
  actList.forEach(function (a) {
    ...
    if (di.model || di.product) {
      // 机型优先取 product（激活 URL 的 m 参数常是 "ap" 这类垃圾值）
      var fp = [tracking.pickModel(di.model, di.product), di.platformVersionCode, di.deviceType, di.screenWidth + "x" + di.screenHeight, di.romVersion, di.language].join("|");
  ...
  if (manualMergeSeeds.length) {
    evidence.push("已应用人工合并规则：" + manualMergeSeeds.join("，") + "（人工规则优先级最高）");
  }
  if (manualSplitValues.length) {
    warnings.push({ level: "info", text: "已应用人工拆分规则，剔除了属于 " + manualSplitValues.join("，") + " 的事件（人工规则优先级最高）" });
  }
  ...
  var user = {
    ...
    activations: actList.map(function (a) {
      ...
      return {
        ...
        device: tracking.pickModel(di.model, di.product), rom: di.romVersion || "", ip: (a.visitor_info || {}).ip || "",
      };
    }),
    lifecycle: lifecycle,
  };
  ...
  return {
    ...
    manual: { merges: manualMergeSeeds, splits: manualSplitValues },
