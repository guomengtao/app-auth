...
function ensureTable() {
  if (!ensurePromise) {
    ensurePromise = (async function () {
      ...
      // ⚠️ 渠道参数（deviceId / utm_* …）原先被整条丢掉：/go/:slug 的 path 是硬编码的
      //    "/go/"+slug，query string 从源头就没进库。这里补两列做长期留存。
      await pg.query("alter table visitor_logs add column if not exists query text not null default ''");
      await pg.query("alter table visitor_logs add column if not exists params jsonb");
...
async function logVisit(entry) {
  ...
  try {
    await ensureTable();
    await pg.query(
      "insert into visitor_logs (ts, ip, path, ua, ref, country, region, city, visitor_hash, source, query, params) " +
      ...
      [
        ...
        String(entry.query || "").slice(0, 500),
        JSON.stringify(entry.params || null),
      ]
    );
    return true;
  } catch (e) {
...
async function listRecent(limit, offset) {
  ...
  var r = await pg.query(
    "select id, ts, ip, path, ua, ref, country, region, city, visitor_hash, query " +
...
//   2) 中文化统一走 geoZh.resolveDisplayGeo()（腾讯组 → 境外源组 → Vercel 头部组，按来源成组）——
//      与「最近访客」完全同源，避免出现「中文市 + 英文州码 = IL · 芝加哥」这种跨源混搭；
//      Vercel 头部组翻译不出省级时回落到国家中文名，绝不把 IL / 06 / Z 这类代码裸奔到界面；
...
async function uniqueIpStats(days, cityFilter) {
  ...
  try {
    ...
    (rows.rows || []).forEach(function (row) {
      // 按来源成组取省市（腾讯组 → 境外源组 → Vercel 头部组），避免「中文市 + 英文州码」混搭
      var pair = geoZh.resolveDisplayGeo({
        region_zh: row.region_zh, city_zh: row.city_zh,
        region: row.region, city: row.city,
        raw_region: row.v_region, raw_city: row.v_city, country: row.v_country,
      });
      var region = stripRegionSuffix(pair.region || "");
      var city = stripRegionSuffix(pair.city || "");
      all.push({
        ip: row.ip,
        visits: row.visits || 0,
        uv: row.uv || 0,
