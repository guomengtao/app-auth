...
function resolveZhLocation(geo) {
  ...
  var regionRaw = String(geo.region || "").trim();
  var region = REGION_ALIAS[regionRaw] || regionRaw;
  var country = String(geo.country || "").trim();

  // zh_region / zh_city：外部已备好的中文（如 ip_lookups 里 ip-api lang=zh-CN 的结果），优先于本地映射
  var cityZh = String(geo.zh_city || "").trim() || lookup(CITY_ZH_MAP, city);
  var regionZh = String(geo.zh_region || "").trim() || lookup(REGION_ZH_MAP, region);

  if (cityZh) return regionZh && regionZh !== cityZh ? regionZh + " " + cityZh : cityZh;
  if (regionZh) return city ? regionZh + " " + city : regionZh;

  var countryZh = lookupCountry(country);
  if (countryZh) return countryZh;   // 海外国家中文名
