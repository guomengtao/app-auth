...
async function loadSwitchesFromUpstash() {
  ...
  for (var i = 0; i < dbs.length; i++) {
    ...
    try {
      ...
      console.warn("[db-switches] Upstash switch fetch failed for", dbs[i].id, ":", e.message || e);
    }
  }
}
  ...
  try {
    ...
    for (var i = 0; i < dbs.length; i++) {
      ...
      try {
        ...
        console.warn("[db-switches] Postgres switch query failed for", dbs[i].id, ":", e.message || e);
      }
    }
  } catch (e) {
    console.warn("[db-switches] Postgres switch load failed:", e.message || e);
  }
}
...
async function savePrimary(name) {
  ...
  if (!edgeConfigId) {
    console.warn("[db-switches] EDGE_CONFIG not set, primary switch will only persist in current instance memory");
  }

  if (edgeConfigId) {
    var vercelToken = process.env.VERCEL_OIDC_TOKEN || process.env.VERCEL_TOKEN || process.env.VERCEL_TOKEN_ALT;
    if (!vercelToken) {
      console.error("[db-switches] No Vercel token found (checked VERCEL_OIDC_TOKEN, VERCEL_TOKEN, VERCEL_TOKEN_ALT). Primary switch will NOT persist to Edge Config.");
      throw new Error("Edge Config write token not configured. Set VERCEL_OIDC_TOKEN, VERCEL_TOKEN, or VERCEL_TOKEN_ALT environment variable.");
    }
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        console.error("[db-switches] Edge Config write failed, status:", resp.status, "body:", errText);
        throw new Error("Failed to write primary to Edge Config (HTTP " + resp.status + "). Primary switch not persisted.");
      }

      console.log("[db-switches] Edge Config updated: " + EDGE_PRIMARY_KEY + " = " + name);
    } catch (e) {
      if (e.message && e.message.indexOf("token not configured") !== -1) throw e;
      console.error("[db-switches] Edge Config write error:", e.message || String(e));
      throw new Error("Edge Config write failed: " + (e.message || String(e)));
    }
  }
