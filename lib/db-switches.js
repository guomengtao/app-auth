...
async function savePrimary(name) {
  ...
  if (name === "upstash") {
    ...
    try {
      ...
      var pool = pg.getNativePool();
      await pool.query(
        "INSERT INTO kv_strings (key, value, expires_at) VALUES ($1, $2, NULL) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
        [PRIMARY_KEY, name]
      );
    } catch (e) {
      // Postgres write is best-effort; cache already updated
    }
  }
