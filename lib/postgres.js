...
function getSupabaseUrl() {
  return process.env.Ev_POSTGRES_URL ||
...
function resolveConnectionString() {
  if (process.env.DB_PROVIDER === "supabase" || process.env.USE_SUPABASE_POSTGRES === "true") {
    if (!dbSwitches.isEnabled("supabase")) {
      if (dbSwitches.isEnabled("neon")) {
        var neonUrl = getNeonUrl();
        if (neonUrl) return neonUrl;
      }
      throw new Error("Supabase is disabled and Neon is not available. No PostgreSQL database enabled.");
    }
    var supabaseUrl = getSupabaseUrl();
    if (!supabaseUrl) {
      console.error("[postgres] DB_PROVIDER=supabase but no Supabase URL found. Set Ev_POSTGRES_URL.");
    }
    return supabaseUrl;
  }
  if (process.env.DB_PROVIDER === "neon" || process.env.POSTGRES_URL) {
    if (!dbSwitches.isEnabled("neon")) {
      if (dbSwitches.isEnabled("supabase")) {
        var supaUrl = getSupabaseUrl();
        if (supaUrl) return supaUrl;
      }
      throw new Error("Neon is disabled and Supabase is not available. No PostgreSQL database enabled.");
    }
    return getNeonUrl();
  }
  if (dbSwitches.isEnabled("supabase")) {
  ...
  throw new Error("All PostgreSQL databases are disabled. Enable at least one in the admin panel.");
}
  ...
  if (!isConfigured()) {
    ...
    client = {
      query: async function (text, params) {
        var isSupabase = process.env.DB_PROVIDER === "supabase" || process.env.USE_SUPABASE_POSTGRES === "true";
        if (!isSupabase) {
          try {
            if (vercelDb) {
              try {
      ...
      sql: function () {
        ...
        var isSupabase2 = process.env.DB_PROVIDER === "supabase" || process.env.USE_SUPABASE_POSTGRES === "true";
        if (isSupabase2) {
