...
async function fetchNeonConsumption() {
  var cfg = getNeonConfig();
  if (!cfg.apiKey || !cfg.projectId) {
    return { configured: false, error: "NEON_API_KEY or NEON_PROJECT_ID not set" };
  }
  try {
    var now = new Date();
    var year = now.getUTCFullYear();
    var month = now.getUTCMonth() + 1;
    var startDate = year + "-" + pad2(month) + "-01";
    var endDate = year + "-" + pad2(month + 1 > 12 ? 1 : month + 1) + "-01";
    if (month === 12) { endDate = (year + 1) + "-01-01"; }

    var url = "https://console.neon.tech/api/v2/projects/" + cfg.projectId + "/consumption?from=" + startDate + "&to=" + endDate;
    ...
    if (!res.ok) {
      ...
      return { configured: true, error: "Neon API returned " + res.status + ": " + text.slice(0, 200) };
    }
    var json = await res.json();
    var totalComputeSeconds = 0;
    var totalDataTransferBytes = 0;
    var totalWrittenDataBytes = 0;
    var totalStorageBytes = 0;
    var periodCount = 0;

    if (json && typeof json.compute_time_seconds === "number") {
      totalComputeSeconds = json.compute_time_seconds;
      totalDataTransferBytes = json.data_transfer_bytes || 0;
      totalWrittenDataBytes = json.written_data_bytes || json.written_size_since_parent_for_child_bytes || 0;
      totalStorageBytes = json.data_storage_bytes_hour || json.logical_size_for_root_bytes || 0;
      periodCount = 1;
    } else if (json && json.data && Array.isArray(json.data)) {
      json.data.forEach(function (item) {
        if (item && item.period) {
          periodCount++;
          if (item.compute_time_seconds) totalComputeSeconds += Number(item.compute_time_seconds) || 0;
          if (item.data_transfer_bytes) totalDataTransferBytes += Number(item.data_transfer_bytes) || 0;
          if (item.written_data_bytes) totalWrittenDataBytes += Number(item.written_data_bytes) || 0;
          if (item.data_storage_bytes_hour) totalStorageBytes += Number(item.data_storage_bytes_hour) || 0;
        }
      });
    }
    ...
    return {
      ...
      totalComputeHours: round(totalComputeSeconds / 3600, 4),
      totalComputeSeconds: totalComputeSeconds,
      totalDataTransferGiB: round(totalDataTransferBytes / (1024 * 1024 * 1024), 4),
      totalWrittenDataGiB: round(totalWrittenDataBytes / (1024 * 1024 * 1024), 4),
      totalStorageBytesHour: totalStorageBytes,
      periods: periodCount,
      raw: json,
      fetchedAt: new Date().toISOString(),
    };
  } catch (e) {
    return { configured: true, error: "Neon API fetch failed: " + (e && e.message ? e.message : String(e)) };
  }
}
...
async function summarize() {
  ...
  var neonCfg = getNeonConfig();
  var neonConsumption = null;
  if (neonCfg.apiKey && neonCfg.projectId) {
    neonConsumption = await fetchNeonConsumption();
  }
  ...
  if (neonConsumption && neonConsumption.configured && !neonConsumption.error) {
    computeResult.neonApiHours = neonConsumption.totalComputeHours;
    computeResult.neonApiSeconds = neonConsumption.totalComputeSeconds;
    computeResult.neonApiDataTransferGiB = neonConsumption.totalDataTransferGiB;
    computeResult.neonApiWrittenDataGiB = neonConsumption.totalWrittenDataGiB;
    computeResult.neonApiPeriods = neonConsumption.periods;
    computeResult.neonApiFetchedAt = neonConsumption.fetchedAt;
    computeResult.note = "Neon API consumption data merged. Official billing takes precedence.";
  }
  ...
  return {
    ...
    neon: {
      apiKeyConfigured: !!neonCfg.apiKey,
      projectIdConfigured: !!neonCfg.projectId,
...
async function getDailyConsumption() {
  try {
    await ensureTables();
    var mk = monthKey(new Date(Date.now()));
    var result = await postgres.query(
      "SELECT DATE(to_timestamp(tick_at / 1000.0)) AS day, COUNT(*)::int AS ping_count, COALESCE(SUM(added_active_seconds),0)::int AS total_active_seconds, COUNT(DISTINCT CASE WHEN gap_seconds >= 300 THEN 1 ELSE NULL END)::int AS cold_starts, COUNT(DISTINCT path)::int AS unique_paths FROM quota_ticks WHERE month_key = $1 GROUP BY day ORDER BY day DESC LIMIT 62",
      [mk]
    );
    return (result && result.rows) ? result.rows : [];
  } catch (e) {
    console.warn("getDailyConsumption failed:", e && e.message ? e.message : e);
    return [];
  }
}
...
module.exports = {
  ...
  summarize: summarize,
  fetchNeonConsumption: fetchNeonConsumption,
  getNeonConfig: getNeonConfig,
