"""Auto Clicker v1.0.5 - Better CLI feedback, first scan always prints, build mode"""
VERSION = "v1.0.5"
    ...
    try:
        ...
        if stderr:
            ...
            if "not found" not in stderr.lower() and "ok" not in stderr.lower():
                if verbose:
                    print(f" -> osascript stderr: {stderr[:80]}")
...
def build_standalone():
    ...
    dist_dir = os.path.expanduser(f"~/Desktop/{app_name}.app/Contents/MacOS")
    os.makedirs(dist_dir, exist_ok=True)

    src = os.path.abspath(__file__)
    dst = os.path.join(dist_dir, "auto_clicker")
    shutil.copy2(src, dst)
    os.chmod(dst, 0o755)

    launcher = os.path.join(dist_dir, "launcher.sh")
    with open(launcher, "w") as f:
        f.write(f'''#!/bin/bash
cd "$(dirname "$0")"
exec /usr/bin/python3 "$(dirname "$0")/auto_clicker"
''')
    os.chmod(launcher, 0o755)

    plist_dir = os.path.expanduser(f"~/Desktop/{app_name}.app/Contents")
    plist = os.path.join(plist_dir, "Info.plist")
...
<dict>
    ...
    print(f"[build] Standalone app created at: ~/Desktop/{app_name}.app")
    print(f"[build] You can double-click it or add to Login Items.")
...
def main():
    ...
    if rumps is None:
        ...
        print("Or use CLI mode: python3 auto_clicker_v1.0.5.py --cli")
        sys.exit(1)
