"""Auto Clicker v1.3.0 - Screen Position Specifier: draggable wireframe overlays with multi-action support"""
VERSION = "v1.3.0"
...
class RegionOverlay:
    ...
    def _draw_everything(self, w, h, color, label):
        ...
        cross_color = self._lighten_color(color)
        self.canvas.create_line(w // 2, 0, w // 2, h, fill=cross_color, dash=(2, 5), tags="cross")
        self.canvas.create_line(0, h // 2, w, h // 2, fill=cross_color, dash=(2, 5), tags="cross")

        rh_size = 18
        self.canvas.create_rectangle(w - rh_size, h - rh_size, w, h,
                                     fill="#3388FF" if self.edit_mode else "", outline="#3388FF",
                                     tags="resize_handle")
        self.canvas.create_line(w - rh_size + 4, h - 4, w - 4, h - rh_size + 4,
                                fill="#FFFFFF" if self.edit_mode else "", width=2, tags="resize_handle")
        self.canvas.create_line(w - rh_size + 4, h - 8, w - 4, h - rh_size + 8,
                                fill="#FFFFFF" if self.edit_mode else "", width=1, tags="resize_handle")

        action_label = f"[{self.cfg.get('action_type', 'click')}]"
        self.canvas.create_text(w // 2, h - 10, text=action_label,
                                fill=color, font=("Helvetica", 8), tags="action_label")
