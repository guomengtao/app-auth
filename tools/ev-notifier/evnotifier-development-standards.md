...
| 禁止项 | 原因 |
|--------|------|
| ❌ 自动轮询 Stream/List | 每5秒一次 = 17280次/天，超过免费额度 |
| ❌ 任何定时 xrange/rpop/lrange | 按时间计费，空转浪费严重 |
| ❌ 后台补偿轮询 | 哪怕每小时1次也是浪费，丢就丢了，用户手动恢复 |
...
| 禁忌 | 说明 |
|------|------|
| 🚫 任何形式的自动轮询 | 定时 xrange/lrange/rpop 一律禁止 |
| 🚫 隐藏 Redis 消耗 | 每次命令调用必须记录到 poll_log |
...
| 项目 | 当前 (v1.5.x) | 目标 (v2.0.0) |
|------|--------------|---------------|
| 通信方式 | Stream XRANGE 轮询 | PUB/SUB 广播 |
| 轮询间隔 | 30秒/次 | 无（禁止自动轮询） |
| Redis 日消耗 | ~2880 次 | 正常 0 次，异常 N 次（手动） |
| 丢失检测 | 有（check_integrity） | 有（total_daily 比对） |
| 丢失恢复 | 自动（每小时最多1次） | 手动（用户点击按钮） |
| 恢复按钮 | 无 | 菜单栏实时显示丢失条数 |
