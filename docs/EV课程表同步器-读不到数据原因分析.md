...
| 现在状态 | **已结案**：v1.0.56 修死锁、v1.0.57 修信封解包+空洞成功、v1.0.58 修 `unwrap_envelope` 误剥+课表宽容拍平；**v1.0.58 复测 ping / export / update_settings 三条链路全部打通**。v1.0.59 把日志框加大 3 倍并新增回包超时提示。手环 EV 版本实测 **1.6.103 / code 932** |
...
### 零之六、v1.0.57 复测：ping 通了，export 又被 `unwrap_envelope` 剥了一层（**已修复 v1.0.58**）

装 v1.0.57 后的日志：

```
[RX] 收到回包 len=81（信封 385）；原文：{"ok":true,"action":"ping","pong":true,"versionName":"1.6.103","versionCode":932}
[RX] ping 回包解析成功（通道双向通）              ← ✅ ping 彻底通了
...
[RX] 收到回包 len=590（信封 1976）；原文：{"ok":true,"action":"export","version":1,"scopes":[…],"data":{"baseFontSize":48,"versionName":"1.6.103","versionCode":932,"nickname":"道饭店","homepag…
[RX] 回包无法识别（可能格式不符）：{"baseFontSize":48,"homepage":{…    ← ❌ 只剩 data 对象了
```

**两个结论**：

1. **手环上 EV 版本 = `1.6.103`（versionCode 932）** —— 相当新，协议 v1 的三个 action 全部支持，R2 彻底排除。
2. export 的**业务回包被多剥了一层**：打印出来的 `inner` 只剩下 `{"baseFontSize":48,…}`，`ok` / `action` / `scopes` 全没了。

**根因**：`unwrap_envelope()` 无条件把最外层 `data` 当成 QAIC 传输层信封剥掉，但 ——

```json
{"ok":true,"action":"export","version":1,"scopes":[…],"data":{…}}
```

**export 业务回包自己就有 `data` 字段**（那是课表数据本身），于是 `action`/`ok` 被连带丢弃 → export 解析失败 → 落到「回包无法识别」。ping 之所以没问题，是因为它的回包**不含 `data` 字段**。

**修复（v1.0.58）**：

| # | 改动 |
|---|---|
| 1 | `unwrap_envelope()` 加前置判定：外层含 `ok` 或 `action` → **它本身就是业务报文，原样返回**；只有"看起来不是业务报文"时才按 `data` 剥 |
| 2 | `ExportData.schedule` 改裸 JSON + 新增 `flatten_schedule()` 宽容拍平：任何字段缺失/类型不符**只跳过那一条**，不会让整包反序列化失败 |
| 3 | `schedule` 非空却拍出 0 条时，打印 `schedule` 原文前 200 字（不静默） |

构建产物：`dist/EV-Schedule-Sync-v1.0.58.abp`（451K）。

> ⚠️ **另一件必须知道的事**：这台手环的 EV 课表当前是**空的**（回包里 `"schedule":[]`），所以即使解析全部修好，界面也只能显示「读到 0 门课程，昵称：道饭店」。要在插件里点「**导入到手环**」（`export-ev-btn`）把课表真正写进手环，再点「读」才有课程。
> 顺带提醒：导入页的「**导入**」按钮（`import-btn`）只是把 JSON 解析进插件内存，**不会发到手环**；发到手环是「导入到手环」。

### 零之五附：v1.0.57 的复测预期（历史记录，已被「零之六」取代）
...
## 零之七、v1.0.58 复测：**三条链路全部打通**（问题结案）

```
[RX] 收到回包 len=81 （信封 385）；原文：{"ok":true,"action":"ping","pong":true,"versionName":"1.6.103","versionCode":932}
[RX] ping 回包解析成功（通道双向通）                                  ✅
[RX] 收到回包 len=593（信封 1985）；原文：{"ok":true,"action":"export",…,"data":{"schedule":[],"nickname":…
[RX] export 回包解析成功（已填充昵称/版本/课程）                      ✅
[RX] 收到回包 len=38 （信封 248）；原文：{"ok":true,"action":"update_settings"}
[RX] 收到 basic 回包（import/update_settings）                        ✅
```

| 能力 | 状态 |
|---|---|
| 通道双向（ping） | ✅ |
| **读手环数据（export）** | ✅ 昵称 / 首页设置 / 版本号 / 课表 全部解析成功 |
| 写配置（update_settings） | ✅ |
| 手环上 EV 版本 | `1.6.103` / code 932（协议 v1 全支持） |

**唯一"看起来空"的地方**：这台手环的 EV 课表本来就是空的（回包 `"schedule":[]`），所以界面显示「读到 0 门课程，昵称：xxx」是**正确结果** —— 先用「导入到手环」写入课表，再读就有课程了。

### 附带观察（v1.0.59 已加对策）

复测日志的后半段出现多次「`[TX] export send=Ok(())` / `update_settings send=Ok(())` 之后没有 `[RX]`」：

- 最可能是**手环息屏 / 回桌面后 EV 被系统挂起**（`system.resident` 未开启），消息没人收；
- 这在日志上原本无法与"插件坏了"区分 —— 因为 `send` 永远返回 `Ok`、QAIC 无 ACK。

**v1.0.59 改动**：

| # | 改动 |
|---|---|
| 1 | 日志框高度 `160 → 480`（**加大 3 倍**，排错时一屏能看到更多步骤） |
| 2 | 每次发消息后登记「待回包」并排 5 秒定时器；超时未被回包覆盖时打印 `[TIMEOUT] …` 并在状态栏给出结论（已发出 N 次 / 收到 M 次 + 三条排查建议），彻底消灭"界面上永远停在已发送" |

构建产物：`dist/EV-Schedule-Sync-v1.0.59.abp`（454K）。

> 至此原始问题（"能启动 EV 但读不到数据"）已彻底解决。剩余待办只有工程性优化：B1 真正的"等注册生效"（目前注册后立即发送，实测无问题）、以及 `interrupt` 之外的其它格式适配。
...
| 已修复到哪一步？ | v1.0.56 修死锁 + C1/C2/C3；v1.0.57 修信封解包 + 空洞成功 + 解析容错 + 诊断日志；v1.0.58 修 `unwrap_envelope` 误剥 + 课表宽容拍平 |
